...
save('NFL_s_bifurcation_diagram_data.mat', 'r_yc', 'x_len', 't_max', 'x_num', 't_num', ...
...
load('NFL_s_bifurcation_diagram_data.mat');
...
grid off;
box on;
%scatter(SF*s1_vals, vals1, 'Marker', 'o', 'MarkerFaceColor', [1, 0, 0], 'MarkerEdgeColor', [1, 0, 0]);
%scatter(SF*s1_vals, vals2, 'Marker', 'o', 'MarkerFaceColor', [1, 0, 0], 'MarkerEdgeColor', [1, 0, 0]);
...
%xlabel('$\textsf{Pheromone signal, s} \left( \frac{\mu m^2}{s \cdot mol.^2} \right)$', 'Interpreter','latex');
ylabel('Total ${C_A}$ (molecules)', 'Interpreter','latex');
%ylabel('Total {C_A} (molecules)');
set(gca, 'FontSize', 30);
xticks(0:0.000001:1*max(s1_vals/SF/SF));
x_ticklabels = string((0:0.000001:1*max(s1_vals/SF/SF))*1000000);
x_ticklabels(1) = "0^{ }"
x_ticklabels(2) = "10^{-6}"
x_ticklabels(3:end) = strcat({x_ticklabels{3:end}}, "\cdot10^{-6}");
xticklabels(x_ticklabels);
xlim([0, 1*max(s1_vals/SF/SF)]);
%yticks(linspace(2500, 8000, 5));
yticks(1500:1500:8000);
set(gca,'linewidth',4);

saveas(fig, 'NFL_s_bifurcation_diagram_total_Ua.fig');
saveas(fig, 'NFL_s_bifurcation_diagram_total_Ua.png');
saveas(fig, 'NFL_s_bifurcation_diagram_total_Ua.pdf');

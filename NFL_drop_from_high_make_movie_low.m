...
inp_file = 'NFL_drop_from_high_and_low.mat';
...
save_fold = inp_fold;
pic_fold = fullfile(save_fold, strcat(strrep(inp_file, '.mat', ''), '_phase_space_pictures_low'));
mkdir(pic_fold);
...
for i = 1:T:size(sol,1)
    ...
    if i < size(sol,1)/2
        ...
        idx = [1:T:size(sol,1)/2, (size(sol,1)/2+1):T:i, i];
    end
    ...
    y_ticks = 0:2000:max(Ua_all(:));
    y_ticklabels = string([0:2000:max(Ua_all(:))]/10000);
    ...
    x_ticklabels = string(round(k1_vals(x_ticks)*100000,2));
...
mov_file = fullfile(save_fold, strrep(inp_file, '.mat', '_dynamics_low'));

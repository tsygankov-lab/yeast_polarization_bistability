...
ex_U_start = 0.9*x_len;
ex_U_end = x_len;
Ua_init_max = U_all/(ex_U_end-ex_U_start);
%Ua_init_max = 0;
Ua_init_min = 0;
...
y_ticks = 0:4000:1.1*max(X_vals_all);
...
saveas(fig1, 'NFL_decrease_from_high_dynamics.fig');
saveas(fig1, 'NFL_decrease_from_high_dynamics.png');
set(fig1, 'PaperPosition', [0.25 0 8.25 8]);
saveas(fig1, 'NFL_decrease_from_high_dynamics.pdf');

save('NFL_decrease_from_high.mat', ...
    'r_yc', 'SF', 'm', 'x_len', 't_max', 'x_num', 't_num', 'x', 't', ...
    'k0', 'k1', 'k2', 'k3', 'k4', 'k5', 'k6', 's1', 's2', 's3', 'drop', ...
    't_sig1', 't_sig2', 'D_Ua', 'D_Ui', 'D_X', 'U_concentration', 'U_all', ...
    'ex_U_start', 'ex_U_end', 'Ua_init_max', 'Ua_init_min', 'Ui_init', ...
    'X_init', 'P', 'opts', 'sol', 'Ua', 'Ui', 'X', 's_vals', 'X_vals', ...
    'Ua_vals', 's', 'X_vals_all');
...
if drop
    ...
    if (t >= t_sig2)
        s = s2 + (s3 - s2)*(t-t_sig2)/(t_max-t_sig2) + mult*x;
    end
end
...
F1 = (k0 + r + (k1+k6*s)*Ua^2)*Ui - (k2+k3*X)*Ua;
F2 = -F1;
F3 = k4*Ua - k5*X;
s = [F1; F2; F3];
